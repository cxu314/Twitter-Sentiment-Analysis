...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']
        t['color'] =colors[int((score+1) * 49.5)]
    return tweets

@app.route("/<name>")
def tweets(name):
    "Display the tweets for a screen name color-coded by sentiment score"
    tweets = add_color(fetch_tweets(api, name)['tweets'])
    score = sorted([tweet['score'] for tweet in tweets])[(len(tweets) + 1) / 2]
    return render_template("tweets.html", tweets=tweets, score=score, name=name)
